...
func (pm *ProtocolManager) buildSnapshot() *Snapshot {
	...
	currBlock := pm.blockchain.CurrentBlock()

	snapshot := &Snapshot{
		addresses:      make([]Address, numNodes),
		removedRaftIds: make([]uint16, numRemovedNodes),
		headBlockHash:  currBlock.Hash(),
	}

	// Populate addresses
	log.Info(fmt.Sprintf("deep|confState Nodes: %+v | snapshot %+v", pm.confState.Nodes, snapshot))
	for i, rawRaftId := range pm.confState.Nodes {
		log.Info("Looping through confstate nodes")
		raftId := uint16(rawRaftId)

		log.Debug(fmt.Sprintf("deep|buildSnapshot i = %d raftid = %d pm = %d snapshot = %+v", i, raftId, pm.raftId, snapshot))
		if raftId == pm.raftId {
...
func (pm *ProtocolManager) applyRaftSnapshot(raftSnapshot raftpb.Snapshot) {
	log.Info("deep:applying snapshot to raft storage")
	...
	log.Info(fmt.Sprintf("snaptshot res from bytesToSnapshot [%+v] [%+v]", snapshot, raftSnapshot.Data))
	...
	latestBlock, _ := pm.blockchain.GetBlockByHash(latestBlockHash)
	if latestBlock == nil {
		pm.logNewlyAcceptedTransactions(preSyncHead)

		log.Info(chainExtensionMessage, "hash", pm.blockchain.CurrentBlock().Hash())
	} else {
		log.Info("deep:snapshot is caught up; no need to synchronize")
		log.Info(fmt.Sprintf("The latest hash is: [%+v]", latestBlockHash))
		log.Info(fmt.Sprintf("The latest block is: [%+v]", latestBlock))
	}
	pm.blockchain.SetHead(latestBlockHash)
	//pm.blockchain.SetHeadBlock(latestBlock)
...
func (pm *ProtocolManager) logNewlyAcceptedTransactions(preSyncHead Block) {
	...
	for currBlock.Hash() != preSyncHead.Hash() {
		...
		blocksSeen += 1
		currBlock, _ = pm.blockchain.GetBlockByHash(currBlock.ParentHash())
	}
