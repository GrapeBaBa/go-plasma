...
type ProtocolManager struct {
	...
	//	downloader Downloader // was: *downloader.Downloader
	minter *minter
	...
	blockProposalC      chan Block             // for mined blocks to raft
...
//func NewProtocolManager(raftId uint16, raftPort uint16, blockchain BlockChain, mux *event.TypeMux, bootstrapNodes []*discover.Node, joinExisting bool, datadir string, minter *minter) (*ProtocolManager, error) {
func NewProtocolManager(raftId uint16, raftPort uint16, blockchain BlockChain, mux *event.TypeMux, bootstrapNodes []*RaftPeer, joinExisting bool, datadir string, minter *minter, block Block) (*ProtocolManager, error) {
	...
	manager := &ProtocolManager{
		...
		eventMux:            mux,
		blockProposalC:      make(chan Block),
...
func (pm *ProtocolManager) Start(p2pServer *p2p.Server) {
	...
	pm.minedBlockSub = pm.eventMux.Subscribe(NewMinedBlockEvent{})
	pm.startRaft()
...
func (pm *ProtocolManager) NodeInfo() *RaftNodeInfo {
	...
	for _, peer := range pm.peers {
		...
		log.Debug(fmt.Sprintf("NodeInfo %d %s", peerIdx, raftUrl(peer.address)))
		peerIdx += 1
	}
	...
	//
	log.Debug(fmt.Sprintf("NodeInfo PeerAddresses %v", peerAddresses))
	return &RaftNodeInfo{
...
func (pm *ProtocolManager) ProposeNewPeer(enodeId string) (uint16, error) {
	//node, err := ParseNode(enodeId)
	node, err := ParseComplete(enodeId)
	if err != nil {
		return 0, err
	}

	if pm.isP2pNodeInCluster(node.Node) {
	...
	for {
		...
		case role := <-roleC:
			...
			if intRole == minterRole {
				EmitCheckpoint(BecameMinter)
				pm.minter.start()
			} else { // verifier
				EmitCheckpoint(BecameVerifier)
				pm.minter.stop()
			}
...
func (pm *ProtocolManager) minedBroadcastLoop() {
	for obj := range pm.minedBlockSub.Chan() {
		...
		case NewMinedBlockEvent:
			log.Debug(fmt.Sprintf("minedBroadcastLoop NewMinedBlockEvent ev = %v", ev))
			select {
	...
	for {
		...
		case block, ok := <-pm.blockProposalC:
			...
			*/
			r, err := block.Encode()
			if err != nil {
				log.Debug("serveLocalProposals encode error", "err", err)
			}

			pm.rawNode().Propose(context.TODO(), r)
		...
		case rd := <-pm.rawNode().Ready():
			...
			for _, entry := range pm.entriesToApply(rd.CommittedEntries) {
				...
				case raftpb.EntryNormal:
					...
					err := rlp.DecodeBytes(entry.Data, pm.blockformat)
					if err != nil {
						log.Error("error decoding block: ", err)
					}

					if pm.blockchain.HasBlock(pm.blockformat.Hash(), pm.blockformat.NumberU64()) && currentBlock.NumberU64() == pm.blockformat.NumberU64() {
						// This can happen:
						//
						...
						log.Debug(fmt.Sprintf("HasBlock Number = %d", pm.blockformat.NumberU64()))
						log.Warn("not applying already-applied block", "block hash", pm.blockformat.Hash(), "parent", pm.blockformat.ParentHash(), "head", headBlockHash)
					} else {
						log.Debug(fmt.Sprintf("applyNewChainHead === %d", time.Now().UnixNano()))
						pm.applyNewChainHead(pm.blockformat)
						log.Debug(fmt.Sprintf("applyNewChainHead *** %d", time.Now().UnixNano()))
					}
...
///////////////// CHECK (snapshot comes first and it doesn't work)
func blockExtendsChain(block Block, chain BlockChain) bool {
	//return block.ParentHash() == chain.CurrentBlock().Hash()
	return true
}

func (pm *ProtocolManager) applyNewChainHead(block Block) {
	if !blockExtendsChain(block, pm.blockchain) {
		...
		if existingBlock, _ := pm.blockchain.GetBlockByHash(block.Hash()); nil == existingBlock {
			if err := pm.blockchain.ValidateBody(block); err != nil {
				panic(fmt.Sprintf("failed to validate block %x (%v)", block.Hash(), err))
			}
		}

		for _, tx := range block.Transactions() {
			EmitCheckpoint(TxAccepted, "tx", tx.Hash().Hex())
			log.Debug(fmt.Sprintf("applyNewChainHead %v", tx))
		}

		_, err := pm.blockchain.InsertChain([]Block{block}) //QUESTION: Why do we send an array of blocks?

		if err != nil {
			panic(fmt.Sprintf("failed to extend chain: %s", err.Error()))
		}
		log.Info(fmt.Sprintf("applyNewChainHead %v", pm.blockchain.CurrentBlock()))

		EmitCheckpoint(BlockCreated, "block", fmt.Sprintf("%x", block.Hash()))
	}
}
...
func ParseComplete(rawurl string) (*RaftPeer, error) {
	var (
		id               discover.NodeID
		ip               net.IP
	...
	if id, err = discover.HexID(u.User.String()); err != nil {
	...
	node := discover.NewNode(id, ip, uint16(udpPort), uint16(tcpPort))
	var raftnode RaftPeer
	raftnode.Node = node
	...
	if qv.Get("raftport") != "" {
		...
		raftnode.RaftPort = uint16(raftPort)
	}
	return &raftnode, nil
}
